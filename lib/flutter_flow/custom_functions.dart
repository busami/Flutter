...
String? completeAddress(AddressStruct? address) {
  if (address == null) {
    return null;
  }
...
List<dynamic> countries(String? countryName) {
  ...
  countryInfoJson.forEach((name, info) {
    if (countryName == null ||
        name.toLowerCase().contains(countryName.toLowerCase())) {
      final countryCodeInLowerCase = info["code"]!.toLowerCase();
      result.add({
        "country": name,
        "dialCode": info["dial_code"]!,
        "flag": 'https://flagcdn.com/h80/$countryCodeInLowerCase.png'
      });
    }
  });
...
Color colorOpacity(
  ...
  // default opsacity 20 and return opcity color based on that opacity value
  final double opacityValue = (opacity ?? 20) / 100.0;
  return color.withOpacity(opacityValue);
}

String? adaptSupabaseToTemplate() {
// Custom Function: adaptSupabaseToTemplate
  dynamic adaptSupabaseToTemplate(dynamic supabaseProperty) {
    if (supabaseProperty == null) return null;

    return {
      // Map your fields to template fields
      'id': supabaseProperty['id'],
      'name':
          '${supabaseProperty['area']} - ${supabaseProperty['property_type']}',
      'price': supabaseProperty['price'],
      'location':
          '${supabaseProperty['governorate']}, ${supabaseProperty['area']}',
      'description': supabaseProperty['description'],
      'images': [], // Will be populated from property_images table
      'agent': supabaseProperty['agent_name'],
      'phone': supabaseProperty['phone_number'],
      'isVerified': supabaseProperty['paci_verified'] ?? false,
      'lat': supabaseProperty['latitude'],
      'lng': supabaseProperty['longitude'],
      // Add default values for missing fields
      'bedrooms': 0,
      'bathrooms': 0,
      'size': supabaseProperty['area_sqm'],
    };
  }
}
