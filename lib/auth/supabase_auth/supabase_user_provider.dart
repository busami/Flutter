...
class BrokerageSupabaseUser extends BaseAuthUser {
  BrokerageSupabaseUser(this.user);
  User? user;
...
Stream<BaseAuthUser> brokerageSupabaseUserStream() {
  ...
  return (!loggedIn
      ...
      .map<BaseAuthUser>(
    (authState) {
      currentUser = BrokerageSupabaseUser(authState?.session?.user);
      return currentUser!;
    },
  );
}
